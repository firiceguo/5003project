...
def RemovePunct(subset):
    ...
    #Replace digit from raw text otherwise it will return error when applying character tagging.
...
def GetSentimentScore(line):
    ...
    sentiment_score = [int(score['neg']*100),int(score['neu']*100),int(score['pos']*100),int(score['compound']*100)]
...
def SentimentFeatureEngineer(selectreviewDF):
    ...
    selectreviewDF = selectreviewDF.withColumn('remove_punc', RemovePunct_udf(selectreviewDF['review_text']))

    tokenizer = Tokenizer(inputCol="remove_punc", outputCol="tokens_word")
    selectreviewDF = tokenizer.transform(selectreviewDF)

    #Remender: Do not combine the structure here otherwise you will get an error. (some columns will not be found)

    selectreviewDF = selectreviewDF.withColumn('num',countTokens_udf(selectreviewDF['tokens_word']))
    selectreviewDF = selectreviewDF.withColumn('filtered_review_text_new', RemoveEmptyEntry_udf(selectreviewDF['tokens_word']))

    selectreviewDF = selectreviewDF.withColumn('Character_List',GetCharacter_List_udf(selectreviewDF['filtered_review_text_new'])) \
                                .withColumn('sentiment_score', GetSentimentScore_udf(selectreviewDF['filtered_review_text_new']))

    selectreviewDF = selectreviewDF.withColumn('Character_vector', list_to_vector_udf(selectreviewDF['Character_List'])) \
                                .withColumn('sentiment_score_vector', list_to_vector_udf(selectreviewDF['sentiment_score']))
...
def UsefulnessPredictionSentment(trainingdata,model):
    ...
    assembler = VectorAssembler(
        inputCols=['num','sentiment_score_vector', 'Character_vector'],
        outputCol="features")
...
if __name__ == '__main__':
    ...
    selectreviewDF = loadDataJson('review', datafrom='mongodb', path=reviewpath, Datalimit=True, DatalimitNum=50)
    ...
    recomlistdDF = GetRecomList(predictions, 'business_id', 'prediction', 2)
