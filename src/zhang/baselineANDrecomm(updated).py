...
def recommsys(training,test,r,it):
    # Build the recommendation model using ALS on the training data
    als = ALS(rank = r,maxIter=it, regParam=0.01, userCol="usr_no", itemCol="bus_no", ratingCol="label")
    # data = df.union(df)
...
if __name__ == '__main__':
    ...
    training, test = recomDF.randomSplit([training_fraction,reserve_fraction, test_fraction])

    
    rmse = recommsys(training,test,30,20)
